...
- The V2 hero reuses the responsive system-map film and optimized WebP posters inside a raised black-bevel card. Never restore the 10 MB 4K source to first paint. The 8-second H.264 files are about 2.1 MB desktop and 1.5 MB mobile; the matching WebP posters are about 67 KB and 62 KB. Desktop removes only enough unused left space to strengthen the 4:3 crop (`translateX(-18%)`); the complete baked Inputs group must remain visible. Inputs, Workflow System, and Outputs belong to the film itself and must not be duplicated in HTML. The only HTML overlay is the Systems Thinking statement, inside the same translated source wrapper. Mobile preserves its native 9:16 film and mobile poster. The app must select the matching poster before loading the matching source. No caption appears below the film. Do not return it to a full-bleed background treatment.
