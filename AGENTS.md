...
- Palette: the homepage base canvas is white (`#FFFFFF`) with ink `#171813`. Bias to Build uses hard gold `#E3A916` and About uses deep forest `#243C31`. The approved accordion is neutral: overlapping charcoal inactive rails open onto a grey-white (`#F1F1ED`) content stage with a slightly darker (`#E7E8E3`) artifact bed. The workflow artwork retains its authored internal black field. Do not reintroduce per-workflow rail colours.
...
- The form, proposal, and annotations share one padded yellow-grey paper object inside the charcoal workspace. Desktop uses opaque warm-paper callout cards in a normal-flow column, short hard-gold connectors, and four numbered proposal-edge anchors; never restore a fixed-coordinate SVG overlay. Copy and containers never cover the proposal. Mobile removes the connectors and stacks the four consistently padded cards below the proposal. The approved rendered PNG is the initial preview fallback; a generated blob PDF takes over after generation.
- The Generate Proposal button explains the multi-minute wait and uses a reduced-motion-safe progress treatment. The outreach example updates its business name, industry language, audience, and local area from the same form state; at rest it uses explicit `[Industry]`, `[Neighbourhood]`, and `[Business name]` placeholders instead of hard-coded dentist or Midtown copy.
