...
- Material: hero and device surfaces share a black bevel, rounded corners, strong physical shadow, and restrained perspective. The hero is the lighter exception: keep its bezel slim and allocate more desktop width to the system film. The portrait keeps its authored brown frame.
...
- Showcase videos begin loading close to their viewport entry, autoplay while visible, and have no visible or invisible pause-button UI. RCCV and Cool Runnings posters, the Cool Runnings shell, and the proposal image use the app's explicit 25% IntersectionObserver deferral because native lazy thresholds fetch them during hero paint. The Okanagan iframe also keeps its URL in `data-src` until its own 25% observer fires. Intrinsic dimensions preserve layout while those assets are deferred.
- The V2 hero reuses the responsive system-map film and optimized WebP poster inside the largest raised black-bevel card. Never restore the 10 MB 4K source to first paint. Desktop deliberately removes the empty left quarter of the 16:9 source inside a 4:3 crop; the video and HTML overlays must remain inside the same translated source wrapper so Inputs, Workflow System, Outputs, and Systems Thinking stay registered. Mobile preserves its native 9:16 source. Approved HTML labels appear only after `playing` and hide immediately on `waiting`, `stalled`, or `pause`. No caption appears below it. Do not return it to a full-bleed background treatment.
