...
- The V2 hero reuses the responsive system-map film and optimized WebP poster inside a raised black-bevel card. Never restore the 10 MB 4K source to first paint. Desktop deliberately removes the empty left quarter of the 16:9 source inside a 4:3 crop; the video and HTML overlays must remain inside the same translated source wrapper so Inputs, Workflow System, Outputs, and Systems Thinking stay registered. Inputs is a complete HTML overlay containing Data & Insights, Audience Signals, Competition, Search Trends, and Brand Rules, rather than only a heading over faint baked text. Mobile preserves its native 9:16 source. Approved HTML labels appear only after `playing` and hide immediately on `waiting`, `stalled`, or `pause`. No caption appears below it. Do not return it to a full-bleed background treatment.
