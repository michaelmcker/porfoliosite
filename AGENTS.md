...
Production telemetry is an explicit release gate. The portfolio has its own standalone Google Analytics account, `Michael McKerracher Portfolio` (`403345825`), and must never live inside Aethereal or another client/company account. Its GA4 property is `michaelmck.site` (`548357242`), web stream `15380359385`, and measurement ID `G-EJ6ZTQDK09`. Every public portfolio page loads that one property through `assets/analytics.js`, with Google Signals and ad-personalization signals disabled. The `sc-domain:michaelmck.site` Search Console property is DNS-verified through Vercel DNS; retain its Google verification TXT record. The canonical sitemap is `https://michaelmck.site/sitemap.xml`. Tests must fail if a tracked page drops the standalone tag, changes the ID, or weakens the proposal-page CSP beyond the required Google Tag and Analytics endpoints.
...
- Palette: the homepage base canvas is white (`#FFFFFF`) with ink `#171813`. Bias to Build uses hard gold `#E3A916` and About uses deep forest `#243C31`. Repeatable Systems is a full-bleed charcoal chapter: overlapping charcoal inactive rails open onto a charcoal active panel, while only the rounded artifact bed uses paper-grey (`#E7E8E3`). The workflow artwork retains its authored internal black field. Do not reintroduce per-workflow rail colours or make the whole chapter light.
...
- Material: hero and device surfaces share a black bevel, rounded corners, strong physical shadow, and restrained perspective. The hero is the lighter exception: keep its bezel slim and use the approved roughly 36/64 copy-to-film desktop balance so the copy does not collapse into a narrow column. The portrait keeps its authored brown frame.
...
- Spacing and type use the V2 tokens in `v2/styles.css`: `--space-section`, `--space-heading`, `--space-copy`, `--type-h1`, `--type-h2`, and `--type-h3`. `Selected Work`, `The repeatable systems behind the work`, `Outcomes`, and `Experience` share one smaller DM Sans chapter-heading treatment; do not turn one back into an oversized editorial serif. The 761–1099px tablet range has its own fluid token values so desktop headings do not survive unchanged into narrow two-column layouts. Do not add one-off heading scales or arbitrary section padding without updating the token contract and viewport proof.
...
- The workflow heading and accordion form one full-bleed charcoal chapter. Active hierarchy is a compact editorial Fraunces title with the CTA in the same top row, a short description, then dominant media on one rounded paper-grey artifact stage. Desktop inactive spines are unnumbered, 64px wide, overlap by 24px, and use ordered perspective, narrow side faces, adjacent charcoal values, and directional shadow so they read as a physical stack. Hover or keyboard focus visibly lifts the chosen rail by 18px and raises its depth; selection slides over 780ms. The active rail is visually hidden so the workflow evidence receives most of the canvas. Below 1100px the perspective resets and the controls become flat rows. Keep explicit numeric flex bases on desktop; transitioning through `auto` makes the slide snap.
...
- Boutique Accommodation uses the same-origin `v2/okanagan-preview/` document inside the wide black browser. Source-lock its public navigation and hero copy to the deployed Okanagan Worker: `Stays`, `Things to Do`, `Gallery`, `Reviews`, `About`, `FAQ`, `Blog`, and `Book your stay`, in that order. Preserve both real Stays cards with their deployed names, summaries, prices, guest counts, and ratings, plus the light/dark logo marks and the four-part proof strip on every hero. It copies the actual deployed Okanagan hero structure and exact source media into three sequential sticky scenes—Overview, Treehouse, Cabin—with semantic, selectable nav/headline/subhead/button HTML over local 854px-wide, fast-start H.264 movies with frequent seek keyframes. Keep each source movie below 2.8 MB. Each scene owns its scroll lock and deterministic forward/reverse seek; scrolling the iframe must not move the portfolio page. The iframe document is always rendered at a fixed 1100px desktop width and scaled into the outer device with a top-left transform, including on 320px and 390px portfolio viewports. This is deliberate: the full desktop navigation must remain visible inside the mock browser rather than reflowing into a phone header. The Stays menu opens, while every preview link prevents navigation. The visible browser URL mirrors the active scene as `okanagantreehouse.ca`, `/treehouse`, or `/cabin` without navigating. Never replace this with flattened screen recordings or a remote iframe. Outer portfolio scroll only moves the shell from a right-hand 3D entry into its final left-tilted pose. Reduced motion keeps all three static semantic scenes and hides their movies.
- Showcase videos begin loading close to their viewport entry, autoplay while visible, and have no visible or invisible pause-button UI. RCCV and Cool Runnings posters, the Cool Runnings shell, and the proposal image remain explicitly deferred because native lazy thresholds fetch them during hero paint, but the heavier proposal image and Okanagan iframe now begin loading at the app's 110% root margin so they are ready before entering the viewport. Intrinsic dimensions preserve layout while those assets are deferred.
- The V2 hero reuses the responsive system-map film and optimized WebP posters inside a raised black-bevel card. Never restore the 10 MB 4K source to first paint. The 8-second H.264 files are about 2.1 MB desktop and 1.5 MB mobile; the matching WebP posters are about 67 KB and 62 KB. Desktop removes only enough unused left space to strengthen the 4:3 crop (`translateX(-18%)`) without cutting off the input routes. Inputs, Workflow System, Outputs, and Systems Thinking use real HTML correction overlays on desktop and tablet, positioned inside the same translated source wrapper so the labels remain crisp and synchronized with the film. The portrait mobile film and poster already contain a complete, legible authored label system, so the HTML correction layer is hidden below 700px to prevent duplicate text, mismatched copy, and unreadably small rows. The app must select the matching poster before loading the matching source and must call `video.load()` to reselect the portrait or landscape source whenever the 699px media query changes; resizing an already-loaded page is part of the QA contract. No caption appears below the film. Do not return it to a full-bleed background treatment.
- Selected Work uses one shared desktop stage height on the white canvas for RCCV, Boutique Accommodation, Cool Runnings, the proposal, and Why Elevators. The chapter begins with a contained navbar-aligned hairline; project stages themselves are transparent. Project labels sit below their titles, and the first stage has no additional rule below the section heading. Tablet layouts use a denser two-column grid for ordinary text-and-device cases, but Boutique remains one column so the scaled 1100px desktop canvas retains a useful device size. Boutique keeps the wide black nested-scroll browser; the curly arrow and `Scroll the preview` occupy a dedicated row, with the label aligned to the line's start and the arrow tip touching the browser's upper edge. No status/Previous/Next UI overlays the work.
...
- The form remains functional. The annotated proposal explains exactly four generated elements: Custom map, Generated sample ad, Unique industry copy, and Offer. They must remain present at every breakpoint without blocking input, preview, Open PDF, or Download controls.
- The proposal workspace is full-bleed white. The form, proposal, and normal-flow annotation column sit directly on that field; never wrap the whole system in another paper card. Individual form and callout surfaces may use warm paper. Four numbered target elements are anchored inside the approved proposal image at the local map, elevator-screen ad, headline copy, and commercial proof/offer. Desktop uses a responsive SVG path layer driven by rendered target and callout rectangles plus `ResizeObserver`; every arrow must terminate on its named target. Copy and containers never cover the proposal. Below 700px, long connector paths disappear, target markers remain on the preview, and the four consistently padded callout cards form a matching numbered list. The approved rendered PNG is the initial preview fallback. A successful request returns a private binary bundle containing a fitted first-page JPEG preview plus the original PDF. The responsive image replaces the fallback in-page; Open PDF and Download keep the PDF blob. Never embed the generated PDF viewer in the page because Chrome's desktop plugin forces mobile pan and zoom.
- The Generate Proposal button explains the multi-minute wait and uses a reduced-motion-safe progress treatment. The outreach example updates its business name, industry language, audience, and local area from the same form state; at rest it uses explicit `[Industry]`, `[Neighbourhood]`, `[Business name]`, and `[number of screens]` placeholders instead of hard-coded dentist or Midtown copy. A successful proposal response returns `X-Proposal-Screen-Count`, and the email replaces the placeholder with the actual unbracketed inventory count.
...
- Remote proposal images must use HTTPS, resolve publicly, return an image MIME type, and remain within their byte cap. The PDF and preview renderers remove Chromium's `--disable-web-security` argument and abort every page request except `about:`, `blob:`, and `data:` URLs. The proposal route has `frame-src 'none'` because the generated preview is an image, not an embedded document.
...
`scripts/qa-v2.mjs` verifies the broader homepage, all five workflow pages, and the V2 local-search case study. `tests/qa-v2-home-breakpoints.mjs` is the focused 1440/1024/768/390/320 regression for hero source/poster selection, duplicate-label prevention, copy/media collision, chapter-heading consistency, horizontal overflow, and the fixed 1100px Okanagan desktop document. `tests/qa-v2-proposal-builder.mjs` verifies the four approved annotations, generated preview bundle, fitted mobile result, and no pan/zoom overflow at the same five widths. Set their documented screenshot-directory variables to save visual evidence.
