...
- Boutique Accommodation uses the same-origin document at `v2/embeds/okanagan-treehouse.html`, built from the live site’s real copy, destinations, and approved local photos. The deployed preview sends `X-Frame-Options: SAMEORIGIN`, so do not point the iframe directly at the Worker or replace the local document with a recording, screenshot, or frame-swap loop. Text must stay selectable, links clickable, and scrolling local to the iframe. Outer portfolio scroll may subtly rotate and settle the browser shell; reduced motion leaves the complete interactive document visible.
...
- Selected Work uses clean white stages for RCCV, Boutique Accommodation, Cool Runnings, the proposal, and Why Elevators. Project labels sit below their titles, and the first stage has no rule below the section heading. Boutique keeps the wide black selectable browser; the curly arrow appears before `Scroll the preview`, both sit above and outside the frame, and no status/Previous/Next UI overlays the work.
...
- The proposal is a larger rounded PDF artifact with no surrounding paper frame. Boutique begins slightly right and rotates gently as outer-page scroll settles it. Cool Runnings uses the graphite laptop cutout: its existing sizzle film sits in the transparent screen aperture and the lid opens with scroll. Why Elevators begins pitched back, finishes at a slight angle by 72% of its scroll progress, and holds that pose before it leaves the viewport.
- About tracks pointer input across the whole section through a V2-only portrait embed inside the approved Renaissance arch. The parent sticky stage is the sole pointer owner and sends normalized coordinates into a presentation-only iframe; the iframe must not install a second cursor or pointer surface. Its 280svh sticky story begins with `Even when the practical value is questionable, there is value in the process.` as one uninterrupted, normally styled sentence. An aria-hidden focus copy is measured onto the exact inline phrase before scroll; only then may the surrounding copy fade and the phrase increase its actual font size, weight, family, and colour. A visible gold line draws toward the portrait, then the process conclusion and build-note link appear before release. Cap the phrase within the copy column and keep the conclusion below it; never use a CSS scale transform or allow either to cross into the portrait.
...
`scripts/qa-v2.mjs` verifies the homepage, all five workflow pages, and the V2 local-search case study at 1440, 1024, 768, 390, and 320 pixels where applicable. It proves Boutique copy can be selected, its in-frame links and document scroll work without moving the portfolio, outer scroll changes the browser pose, the Cool Runnings laptop opens, showcase films advance on HTTP and the local `file://` preview, accordion widths interpolate, and the About focus copy begins on the exact inline anchor without crossing the portrait. For the contact finale it proves idle state below the visibility threshold, viewport alignment and wheel locking during the autonomous entrance, scroll restoration before gravity, entrance spacing, crossings through both inward turns, continuous size attenuation, 70% distributed convergence, position- and scale-stable physics release, once-only playback, early copy resolution, both loaded portrait layers, dynamic movement or rotation after drag release, active boundaries, normal footer access, and the reduced-motion fallback. Set `QA_V2_SCREENSHOT_DIR=/absolute/path` to save full-page captures, the mid-spiral state, all five accordion states, and the settled finale.
