...
- The V2 hero reuses the 1080p responsive system-map film and optimized WebP poster inside the largest raised black-bevel card. Never restore the 10 MB 4K source to first paint. Approved HTML overlay labels cover the film's unfinished regions and no caption appears below it. Do not return it to a full-bleed background treatment.
...
- The proposal workspace is full-bleed charcoal. The form, proposal, and normal-flow annotation column sit directly on that chapter; never wrap the whole system in another paper card. Individual form and callout surfaces may use warm paper. Every desktop connector and its numbered circle are one callout-owned unit on the same 50% axis, so breakpoints cannot separate the number from the arrow. Never restore independent fixed-percentage pins or a fixed-coordinate SVG overlay. Copy and containers never cover the proposal. Mobile removes connector graphics and uses the four consistently padded callout cards as a numbered list below the proposal. The approved rendered PNG is the initial preview fallback; a generated blob PDF takes over after generation.
- The Generate Proposal button explains the multi-minute wait and uses a reduced-motion-safe progress treatment. The outreach example updates its business name, industry language, audience, and local area from the same form state; at rest it uses explicit `[Industry]`, `[Neighbourhood]`, `[Business name]`, and `[30]` screen-count placeholders instead of hard-coded dentist or Midtown copy. A successful proposal response returns `X-Proposal-Screen-Count`, and the email replaces `[30]` with the generated inventory count.
