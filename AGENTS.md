...
- The contact chapter is a progressive-enhancement sequence. Scroll drives a deterministic, reversible, full-frame curved orbit from the upper left, down the left, across the bottom, up the right, and into the upper middle. At `0.68` progress, the exact DOM poses transfer without a visible jump into locally vendored Matter.js bodies for one short gravity drop with restrained bounce and rotation. Contact copy resolves about 420ms into that drop and switches between dark and light treatments according to object overlap. After settling, dragging an object pins it exactly where it is released; it must not acquire throw velocity, fall again, bounce, or keep spinning. Use the actual laptop, browser, proposal, and workflow assets plus the layered head-visible finale portrait rather than placeholder cards. Reduced-motion and failed-script states show the complete hard-gold contact composition without the interaction.
...
`scripts/qa-v2.mjs` verifies the homepage, all five workflow pages, and the V2 local-search case study at 1440, 1024, 768, 390, and 320 pixels where applicable. It proves the accommodation browser locally captures wheel scrubbing while the page scrolls normally outside it, showcase films advance on HTTP and the local `file://` preview, accordion widths interpolate, the About phrase and single parent-owned portrait pointer remain stable, the contact orbit reaches its sampled full-frame positions and reverses deterministically, physics releases without a positional jump, the copy resolves during the drop, the finale portrait has both loaded layers, dragged objects remain pinned, the footer releases, and reduced-motion exposes the static fallback. Set `QA_V2_SCREENSHOT_DIR=/absolute/path` to save full-page captures, all five accordion states, and the settled finale.
