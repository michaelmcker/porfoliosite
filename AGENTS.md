...
- The contact chapter is a progressive-enhancement sequence: deterministic reversible scroll spiral, one-way release into locally vendored Matter.js gravity, settled draggable/tossable portfolio objects, then a text morph from `Systematise.` to `Let’s build something useful.` before normal scrolling releases into the footer. Physics bodies must inherit the final spiral coordinates without a visible jump. Use the actual portrait, laptop, browser, proposal, and workflow assets rather than placeholder cards. Reduced-motion and failed-script states show the complete hard-gold contact composition without the interaction.
...
`scripts/qa-v2.mjs` verifies the homepage, all five workflow pages, and the V2 local-search case study at 1440, 1024, 768, 390, and 320 pixels where applicable. It proves the accommodation browser locally captures wheel scrubbing while the page scrolls normally outside it, showcase films advance on HTTP and the local `file://` preview, accordion widths interpolate, the About phrase and single parent-owned portrait pointer remain stable, the contact spiral reverses deterministically, physics releases without a positional jump, settled objects drag, the text morph resolves, the footer releases, and reduced-motion exposes the static fallback. Set `QA_V2_SCREENSHOT_DIR=/absolute/path` to save full-page captures, all five accordion states, and the settled finale.
