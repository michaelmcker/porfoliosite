...
- Showcase videos begin loading close to their viewport entry, autoplay while visible, and have no visible or invisible pause-button UI. Below-fold images use native lazy loading plus intrinsic dimensions so deferral does not create layout shift.
- The V2 hero reuses the 1080p responsive system-map film and optimized WebP poster inside the largest raised black-bevel card. Never restore the 10 MB 4K source to first paint. Preserve the desktop film at 16:9 and the mobile film at 9:16; the overlay must share the exact video rectangle rather than a cropped 4:3 box. Approved HTML labels cover the film's unfinished regions, appear only after `playing`, and hide immediately on `waiting`, `stalled`, or `pause`. No caption appears below it. Do not return it to a full-bleed background treatment.
