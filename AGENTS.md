...
- Spacing and type use the V2 tokens in `v2/styles.css`: `--space-section`, `--space-heading`, `--space-copy`, `--type-h1`, `--type-h2`, and `--type-h3`. `Selected Work`, `The repeatable systems behind the work`, and `Outcomes` share one smaller DM Sans chapter-heading treatment; do not turn one back into an oversized editorial serif. The 761–1099px tablet range has its own fluid token values so desktop headings do not survive unchanged into narrow two-column layouts. Do not add one-off heading scales or arbitrary section padding without updating the token contract and viewport proof.
...
- The V2 hero reuses the responsive system-map film and optimized WebP poster inside a raised black-bevel card. Never restore the 10 MB 4K source to first paint. Desktop removes only enough unused left space to strengthen the 4:3 crop (`translateX(-18%)`); the complete Inputs group must remain visible. Video and HTML overlays stay inside the same translated source wrapper. Replacement labels use individual text-sized paper masks aligned to the baked words, never one opaque rectangle over the routes. Inputs contains Data & Insights, Audience Signals, Competition, Search Trends, and Brand Rules. Mobile preserves its native 9:16 source and uses its own label coordinates. Approved labels remain hidden until the first decoded `loadeddata` frame and stay visible through playback stalls once that frame exists. No caption appears below the film. Do not return it to a full-bleed background treatment.
...
`scripts/qa-v2.mjs` verifies the homepage, all five workflow pages, and the V2 local-search case study at 1440, 1024, 768, 390, and 320 pixels where applicable. It proves native 16:9 desktop and 9:16 mobile hero geometry, label/video rectangle parity, playback-state label gating, the Okanagan scene order, sticky lock, desktop nav at embedded width, active-scene URL labels, visible/selectable semantic copy, forward and reverse movie seeking, functional non-navigating Stays menu, iframe-only scroll, outer-shell pose, direct `file://` playback, equal Selected Work desktop heights, and Cool Runnings completion at text centre. It also verifies cue clearance, showcase autoplay, accordion interpolation, mobile and desktop About anchors/resolution, and the desktop/mobile contact-finale physics contract. Set `QA_V2_SCREENSHOT_DIR=/absolute/path` to save the complete evidence set. Add `QA_V2_SCREENSHOT_MODE=lean` for compact Hero/Selected Work, Boutique, Cool Runnings, mobile About, and mobile-finale proof captures while retaining the full runtime checks.
