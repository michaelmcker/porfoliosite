...
- `Selected Work`, `The repeatable systems behind the work`, and `Outcomes` share the same smaller DM Sans chapter-heading treatment. Fraunces remains reserved for selected editorial and personal moments.
...
- Reuse the responsive system-map film in the raised black-bevel hero card, beside the hero copy. Desktop uses `translateX(-18%)` so unused source space is reduced while the complete Inputs group remains visible. Keep the video and HTML masks inside the same source wrapper. Inputs, Workflow System, and Outputs use individual text-sized paper masks placed over the baked words; no opaque label-region box may cover routes. Inputs contains Data & Insights, Audience Signals, Competition, Search Trends, and Brand Rules. Mobile resets the wrapper, preserves native 9:16, and uses mobile-specific label coordinates. Masks appear only after the first decoded frame and remain visible through temporary playback stalls. The hero subhead ends after `scale.`; `And sometimes, stuff just for fun.` is a separate italic Fraunces beat.
