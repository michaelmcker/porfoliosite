...
- Reuse the responsive system-map film in the largest raised black-bevel hero card, beside the hero copy. Preserve the desktop source at 16:9 and the mobile source at 9:16 so HTML masks share the exact media box. Overlay labels appear only once playback fires `playing` and disappear immediately during `waiting`, `stalled`, or `pause`. The hero subhead ends after `scale.`; `And sometimes, stuff just for fun.` is a separate italic Fraunces beat.
...
- Hero, RCCV, and Cool Runnings videos have no visible or invisible Pause control surface. The hero uses the 1080p film and a 67 KB WebP poster rather than loading the 10 MB 4K source. Its real HTML masks are playback-gated and anchored in the native source aspect ratio, not an `object-fit` crop. Below-fold films and images lazy-load close to entry, carry intrinsic dimensions to prevent layout shift, and autoplay only while visible; reduced-motion users see static posters. The Okanagan iframe and its later scene movies are deferred independently.
...
- Public explanation is limited to four accurate outputs: the custom proposal, its current per-screen pricing, nearby available screens, and the monthly impression estimate carried by that inventory package.
- Desktop uses a full-bleed white working area with the live form, a large proposal object, and opaque warm-paper callout cards in a normal-flow column. The system sits directly on that white field; there is no outer paper wrapper. Custom proposal points to the tailored creative; Screens, Impressions, and Pricing point to the three proof values in the lower-left column immediately beside the map. The map remains visible but is no longer its own callout. A responsive SVG layer calculates curves from each callout to those rendered targets and updates through `ResizeObserver`. Mobile hides the long paths, keeps the four PDF targets, and uses an 18px-padded matching numbered card list. A successfully generated blob PDF replaces the fallback image.
...
- The hero's approved HTML overlay labels remain anchored over unfinished diagram regions at every breakpoint by preserving native 16:9 and 9:16 video geometry. They never appear before playback.
