...
- Use local DM Sans for functional copy. Use local Fraunces for project titles, workflow titles, metrics, and selective personal/editorial moments. Desktop workflow controls are 88px charcoal spines fixed on the right with vertical, heavy DM Sans labels. The selected workflow opens as a separate loose paper card to their left. Tablet and mobile retain the existing overlapping vertical folder rows and horizontal labels.
...
- Desktop workflow states use a full-width fixed-spine composition at 1100px and wider; smaller layouts retain the existing vertical folder rows with mobile-readable artwork. The five neutral spines remain in permanent right-hand slots. The loose paper retracts right behind them before the next paper slides left into view; changing selection must produce zero horizontal movement in every trigger.
