...
V2 presents this as one full-bleed charcoal chapter: the centred section heading leads directly into a contained neutral dark accordion. Desktop uses five separate smoothly rounded charcoal folder tabs in permanent top-to-bottom slots on the right, with a narrow gap and vertical Fraunces labels. The tabs stay behind the paper at rest, keeping its edge smooth; hover or focus lifts one tab 12px outward with a deeper shadow. Desktop overflow remains visible so that lift is never clipped by the component boundary. The active workflow appears on one loose `#F7F6F2` warm-paper card to the left with no folder pocket or holder. When selection changes, the current paper retracts rightward behind the tab column for 200ms and the next paper slides left into place over roughly 520ms. Mobile keeps the existing overlapping vertical folder rows and immediate interaction. Do not restore per-workflow folder colours. Full-background exports stay intact on the paper surface, and transparent artwork never resolves onto black. The approved Search-Optimized Content plate defines the artwork system for the process workflows: black canvas, off-white cards, recognisable tool logos, realistic artifact fragments, and blue/green/red/gold connecting routes. Presentation, Prospecting, and Website Production use that language with workflow-specific shapes. Agency Dashboard uses the recovered real V3 AI Visibility Dashboard screenshot.
...
- Desktop source truth is the user-supplied approved plate at `v2/assets/workflows/content-production-approved-desktop.png`, SHA-256 `8df1ac85b4813445e5ffd836efcd8cce380a945b2a2e315fcead1c297bb1932b`. Never substitute the older generic diagram, crop it, or regenerate it. Production uses `content-production-approved-desktop.webp`, SHA-256 `7cd3ae11631bb49ceea9e40e7dafd658a770cc918c2b21754f8d8dfd26d1fb11`, unchanged on the shared warm artwork surface.
...
- Desktop folder controls follow `docs/references/workflow-folder-tabs-desktop-rest.png`: five horizontal tabs stay in one top row and the warm sheet starts beneath them. `docs/references/workflow-folder-tabs-desktop-page-turn.png` locks the interaction: only the paper turns right-to-left, revealing the next sheet underneath; tabs stay fixed. The current timing is 620ms. Hover raises a tab 10px without selecting it. The prior right-side vertical spine and retract/slide interaction are rejected.
...
- Use local DM Sans for functional copy. Use local Fraunces for project titles, workflow titles, metrics, selective personal/editorial moments, and the desktop folder labels. Desktop workflow controls are five discrete shaped charcoal tabs fixed down the right with top-to-bottom vertical labels. The selected workflow opens as a separate loose warm-paper card to the left. Tablet and mobile retain the existing overlapping vertical folder rows and horizontal labels.
...
- Do not use pastel or unrelated flat fields as the homepage structure. Bias to Build uses hard gold and About uses deep forest. The approved Repeatable Systems chapter is charcoal: five discrete shaped charcoal folder tabs form one fixed right-hand column beside one loose rounded paper-grey workflow card. Do not add a literal folder holder. The same paper-grey surface surrounds artwork on workflow detail pages. Retain complete authored image backgrounds and never let transparent artwork resolve onto black. Do not colour-code individual workflows.
- Field Notes is private workshop tooling. Preserve its source, approval rules, renderer, and local output, but keep it absent from navigation, RSS discovery, the public sitemap, production promotion, and Vercel deployment until a separate publishing decision restores it.
...
- Reuse the responsive system-map film in the raised black-bevel hero card, beside the hero copy. Desktop uses `translateX(-18%)` so unused source space is reduced while the complete input routes remain visible. Inputs, Workflow System, Outputs, and Systems Thinking use real HTML correction overlays inside the same translated wrapper on desktop and tablet. The overlay layer appears with the first decoded frame so labels do not arrive before motion. The app selects the 67 KB desktop or 62 KB mobile WebP poster before loading the matching 2.1 MB landscape or 1.5 MB portrait H.264 source. Mobile resets the wrapper, preserves native 9:16, and uses the complete authored labels already present in the portrait poster and every film frame; hide the HTML correction layer below 700px to avoid duplicate, mismatched, or undersized text. When a loaded page crosses the 699px breakpoint, update the poster, call `video.load()` so the browser reselects the responsive source, and resume a visible film; do not assume `<source media>` will switch by itself. The hero subhead ends after `scale.`; `And sometimes, stuff just for fun.` is a separate italic Fraunces beat.
...
- Keep full-resolution PNG artwork as source truth, not as the default browser payload. Public display references use quality-90 WebP derivatives with matching dimensions and alpha where required. The homepage responsive still-image budget is under 4.5 MB in total and 350 KB per file; the 2026-08-06 pass reduced the referenced set from 20.72 MB to 2.89 MB without changing the approved compositions.
...
- Desktop workflow states use a full-width fixed-tab composition at 1100px and wider; smaller layouts retain the existing vertical folder rows with mobile-readable artwork. The five neutral folder silhouettes remain discrete in permanent top-to-bottom slots down the right. The loose paper retracts right behind them before the next paper slides left into view; changing selection must produce zero movement in every trigger and label.
...
- About uses the Renaissance arch. `Even when the practical value is questionable, there is value in the process.` is one normal sentence at rest. A lightweight layered portrait fills the frame while the interactive iframe initializes. An aria-hidden duplicate is measured onto the exact start of the practical-value clause; scroll then fades the original, interpolates real font size, weight, family, and colour for sharp text, keeps the growing copy inside its column, resolves the process conclusion below it, draws a visible gold line toward the portrait, and only then releases the page. The build note explains the stable base, 66 registered poses, pointer mapping, typing loop, compression, and compositing constraints. Mobile keeps a real 240svh sticky story rather than a static fallback: the phrase caps at 2.35rem and interpolates from its real wrapped position to a viewport-safe target, the bottom-right portrait grows to 50vw with a 210px cap, and the conclusion resolves above it at `47svh`. The shorter line ends at the frame edge. Opening the build note hides the animated phrase, portrait, and route beneath an opaque inset panel with its own capped scrolling, preventing any collision with readable copy. A stable parent stage exclusively owns the pointer and sends normalized coordinates into the presentation-only portrait iframe. Experience is a tighter centred ledger without row rules, route dots, or timeline markers.
...
- Keep service boundaries explicit: GA4 records site events; it is not a credential store or API orchestration layer. LetzAI and Mapbox run only in Vercel server functions for proposal generation. Search Console, GA4 Data API, and DataForSEO run only in the GitHub Actions refresh job. Their credential values stay in Vercel or GitHub Secrets; browser-visible code receives only same-origin proposal responses and the sanitized aggregate `data/cool-runnings-metrics-current.json` snapshot.
...
- The hero uses real HTML correction overlays for Inputs, Workflow System, Outputs, and Systems Thinking on desktop and tablet, all anchored inside the media wrapper. Below 700px the correction layer stays hidden because the portrait poster and film already contain a complete authored mobile label system. The app selects matching desktop/mobile posters and sources before loading and reveals the desktop/tablet label layer only after a decoded frame exists.
