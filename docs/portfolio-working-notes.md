...
- Reuse the responsive system-map film in the largest raised black-bevel hero card, beside the hero copy. Desktop crops the empty left quarter of the 16:9 film into a 4:3 card. Keep the video and HTML masks inside the same translated source wrapper so the Inputs mask remains visible at the crop edge. Mobile resets the wrapper and preserves the native 9:16 source. Overlay labels appear only once playback fires `playing` and disappear immediately during `waiting`, `stalled`, or `pause`. The hero subhead ends after `scale.`; `And sometimes, stuff just for fun.` is a separate italic Fraunces beat.
...
- Hero, RCCV, and Cool Runnings videos have no visible or invisible Pause control surface. The hero uses fast-start 1080p re-encodes and a 67 KB WebP poster rather than loading the 10 MB 4K source. Desktop film and masks share one translated 16:9 source wrapper inside the 4:3 crop; mobile remains native 9:16. RCCV and Cool Runnings posters, the Cool Runnings shell, the proposal image, and the Okanagan iframe use explicit 25% IntersectionObserver deferral because native lazy loading fetched them during hero paint. Intrinsic dimensions prevent layout shift, and films autoplay only while visible; reduced-motion users see static posters. The Okanagan scene movies are deferred independently.
...
- The hero's approved HTML overlay labels remain anchored over unfinished diagram regions at every breakpoint. Desktop translates the 16:9 video and masks together inside the 4:3 crop; mobile preserves native 9:16 geometry. They never appear before playback.
