...
- Desktop uses a full-bleed charcoal working area with the live form, a large proposal object, and opaque warm-paper callout cards in a normal-flow column. The system sits directly on the charcoal chapter; there is no outer paper wrapper. Every thin hard-gold arrow and numbered circle is one callout-owned unit on a shared vertical axis; no independent percentage pin, annotation copy, or box sits on the PDF. Mobile removes connector graphics and uses an 18px-padded numbered card list. A successfully generated blob PDF replaces the fallback image.
- The outreach email keeps adjustable values in brackets, including `[30]` as the sample screen count. A successful generation returns `X-Proposal-Screen-Count`, and the email updates the bracketed count from the same inventory used in the PDF.
