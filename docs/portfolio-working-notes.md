...
V2 presents this as one full-bleed charcoal chapter: the centred section heading leads into the original live sliding-rail accordion. Desktop keeps four inactive charcoal rails visible while the selected rail collapses and its panel consumes the remaining width. Resting rails are 76px wide with only 12px overlap, so every vertical label is readable without hover. Hover or focus keeps the live site's restrained 3D leftward lift. Selection animates the rails and panel together over 780ms. Below 1100px the component becomes the live vertical accordion with 72px rows and horizontal labels. Do not restore per-workflow colours or the fixed folder-card experiment. Workflow images remain on the shared `#F7F6F2` warm-paper surface, and transparent artwork never resolves onto black. The approved Search-Optimized Content plate defines the artwork system for the process workflows: black canvas, off-white cards, recognisable tool logos, realistic artifact fragments, and blue/green/red/gold connecting routes. Presentation, Prospecting, and Website Production use that language with workflow-specific shapes. Agency Dashboard uses the recovered real V3 AI Visibility Dashboard screenshot.
...
- Use local DM Sans for functional copy and desktop workflow rail labels. Use local Fraunces for project titles, workflow titles, metrics, and selective personal/editorial moments. Desktop workflow controls are the live sliding flex rails with readable vertical labels. Tablet and mobile use full-width vertical rows with horizontal labels.
...
- Do not use pastel or unrelated flat fields as the homepage structure. Bias to Build uses hard gold and About uses deep forest. The approved Repeatable Systems chapter is charcoal and uses the original live sliding rails; do not restore fixed folder tabs or add a literal folder holder. The paper-grey surface surrounds workflow artwork on the homepage and detail pages. Retain complete authored image backgrounds and never let transparent artwork resolve onto black. Do not colour-code individual workflows.
...
- Desktop workflow states use the original full-width sliding-rail composition at 1100px and wider; smaller layouts use vertical rows with mobile-readable artwork. Changing selection collapses the chosen rail and restores the previous rail in document order. Every inactive rail label must remain readable at rest and keyboard selection must preserve the same transition and accessible state.
