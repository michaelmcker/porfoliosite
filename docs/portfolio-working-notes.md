...
- Reuse the responsive system-map film in the raised black-bevel hero card, beside the hero copy. Desktop crops the empty left quarter of the 16:9 film into a 4:3 card. Keep the video and HTML masks inside the same translated source wrapper. Inputs is a full HTML mask with Data & Insights, Audience Signals, Competition, Search Trends, and Brand Rules, not a heading over faint baked rows. Mobile resets the wrapper and preserves the native 9:16 source. Overlay labels appear only once playback fires `playing` and disappear immediately during `waiting`, `stalled`, or `pause`. The hero subhead ends after `scale.`; `And sometimes, stuff just for fun.` is a separate italic Fraunces beat.
