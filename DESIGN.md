...
The workflow accordion is deliberately neutral and dark: Repeatable Systems is a full-bleed charcoal chapter, overlapping folder sheets use adjacent charcoal values with warm-white type, and the active content remains charcoal. Every workflow image sits on the same rounded paper-grey `#E7E8E3` card on the homepage and detail pages. Full-background exports retain their complete authored canvas inside that card; background-removed or transparent assets must reveal paper grey, never black. Bias to Build uses hard gold and About uses deep forest. The Hero and Selected Work are white; a contained hairline aligned to the navbar width separates those chapters without creating a full-bleed colour block. Project stages remain transparent. Music is the other black interlude.
...
- Folder labels: local DM Sans at 900 weight, set horizontally inside raised tabs. Desktop uses 112px overlapping folder sheets with adjacent charcoal values and ordered depth; hover and keyboard focus lift a folder toward the reader without changing selection. The selected tab remains visible and joins the open folder body. Below 1100px the same labels become raised lips on overlapping vertical folder rows.
