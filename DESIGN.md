...
The workflow accordion is deliberately neutral and dark: Repeatable Systems is a full-bleed charcoal chapter using the original live sliding rails. The active panel keeps its title and copy on charcoal, with the dominant workflow image on a warm-paper `#F7F6F2` surface. Workflow detail pages use the same artwork surface. Full-background exports retain their complete authored canvas; background-removed or transparent assets must reveal warm paper, never black. Bias to Build uses hard gold and About uses deep forest. The Hero and Selected Work are white; a contained hairline aligned to the navbar width separates those chapters without creating a full-bleed colour block. Project stages remain transparent. Music is the other black interlude.
...
- Workflow rail labels: local DM Sans at 850 weight. Desktop inactive rails are 76px wide with 12px overlap, leaving a 64px readable face for every vertical label without hover. The active rail collapses as its panel expands; the previous active rail slides back into the stack. Hover and focus use the live site's subtle 3D leftward lift. Below 1100px labels become horizontal on 72px accordion rows.
...
- The neutral workflow accordion becomes vertical below 1100px. On desktop, selection animates by transitioning each rail's flex basis, width, and opacity over 780ms with `cubic-bezier(.22, 1, .36, 1)`. The selected rail closes as its panel opens; the previous active rail reappears in document order. Inactive panels remain hidden and cannot intercept input. Mobile and reduced-motion keep the active row visible as the accessible control.
