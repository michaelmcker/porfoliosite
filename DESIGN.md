...
The workflow accordion is deliberately neutral: overlapping inactive rails use charcoal (`#202422`) with warm-white type and all active content sits on one charcoal stage. Bias to Build uses hard gold and About uses deep forest. The Hero and Selected Work are white; a contained hairline aligned to the navbar width separates those chapters without creating a full-bleed colour block. Project stages remain transparent. Music is the sole black interlude.
...
- Inactive desktop rail names: local DM Sans at 900 weight and `1.2rem` within 64px layered spines. Spines overlap by 24px and use ordered perspective, narrow side faces, adjacent charcoal values, and directional shadow; the active rail hides so workflow evidence keeps most of the canvas. Below 1100px the controls reset to flat rows.
...
`v2/proposal-generator.html` keeps the V2 editorial hierarchy while presenting the real proposal as the evidence. The introduction is open white space; the working form and annotated proposal occupy one full-bleed charcoal workspace; the outcome resolves on hard gold. The page must not inherit the V1 proposal stylesheet.

The proposal is a large, lightly tilted paper object. Desktop annotations are opaque warm-paper cards in the dark margins and use thin curved hard-gold arrows for custom ad, custom copy, live screen count, and local map. The arrows stop at the paper edge; annotation text and containers must never overlap the sheet or leave the viewport. Below 700px, the arrows disappear and four gold pins connect the proposal to consistently padded stacked cards. The approved PNG is the reliable initial visual at every breakpoint, while the generated blob PDF replaces it after a successful submission.
...
The visual layer reuses the shared proposal client and API contract rather than imitating generation. Inputs remain selectable, text remains copyable, and open/download actions remain functional. The strict proposal CSP and browser-security headers apply to both V1 and V2 proposal routes without affecting other embedded V2 experiences.
