...
- Folder labels: local DM Sans at approximately 820 weight. Desktop uses narrow 88px charcoal spines arranged horizontally with the labels reading vertically; hover and keyboard focus lift a spine toward the reader without changing selection. The active paper card sits beside the spines as an independent object. Below 1100px preserve the existing horizontal labels on overlapping vertical folder rows.
...
- The neutral workflow accordion becomes vertical below 1100px. On desktop, the current paper retracts for 200ms, then the selected paper begins to the right and slides left into place over roughly 520ms with `cubic-bezier(.22, 1, .36, 1)`. Desktop rails transition between explicit numeric flex bases; never transition through `auto`, which causes the active panel to jump. Inactive panel canvases must collapse to zero width and remain hidden while their spines move. Mobile and reduced-motion selection commit immediately and keep the active row visible as the accessible control.
