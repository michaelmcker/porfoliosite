...
The workflow accordion is deliberately neutral and dark: Repeatable Systems is a full-bleed charcoal chapter with five discrete charcoal folder tabs fixed in one tightly spaced vertical column on the right and warm-white vertical labels. The active content is one loose warm-paper `#F7F6F2` card to the left that travels independently, never a sheet inside a folder holder. Workflow detail pages use the same warm-paper artwork surface. Full-background exports retain their complete authored canvas; background-removed or transparent assets must reveal warm paper, never black. The desktop component permits visible overflow so the 12px hover lift remains visible beyond its right edge. Bias to Build uses hard gold and About uses deep forest. The Hero and Selected Work are white; a contained hairline aligned to the navbar width separates those chapters without creating a full-bleed colour block. Project stages remain transparent. Music is the other black interlude.
...
- Folder labels: local Fraunces at approximately 520 weight. Desktop uses five separate, smoothly rounded folder-tab silhouettes in permanent top-to-bottom slots on the right with a narrow gap. Labels read vertically from top to bottom. Selection never moves their resting slots; hover and focus lift one tab 12px outward. Tabs layer behind the paper so its right edge remains uninterrupted. The contained active paper card sits to the left as an independent object. Below 1100px preserve the existing horizontal labels on overlapping vertical folder rows.
...
- The neutral workflow folder becomes vertical below 1100px. On desktop, all five discrete shaped tabs sit in one fixed horizontal row across the top, with normal horizontal Fraunces labels and a continuous folder baseline. The active warm-paper sheet begins directly below the tab row. Selection turns the outgoing sheet right-to-left around its left binding edge over 620ms while the incoming sheet remains visible underneath; tabs never move with the sheet. Hover lifts a tab by 10px without changing selection. The binding references are `docs/references/workflow-folder-tabs-desktop-rest.png` and `docs/references/workflow-folder-tabs-desktop-page-turn.png`. Mobile and reduced-motion selection remain immediate and keep the active row visible as the accessible control.
...
- The V2 hero uses the responsive system-map film and matching optimized WebP posters inside a raised black-bevel card beside the copy; the 10 MB 4K source is not a first-paint asset. The checked-in 8-second H.264 files are about 2.1 MB desktop and 1.5 MB mobile, with approximately 67 KB and 62 KB first-paint posters. Desktop uses a restrained `-18%` translation so unused source space is reduced without clipping the input routes. Inputs, Workflow System, Outputs, and Systems Thinking use real HTML correction overlays inside the translated source wrapper on desktop and tablet. Those overlays remain hidden until the first decoded frame so text and motion arrive together. Mobile resets the wrapper, preserves native 9:16, and relies on the complete authored labels in the portrait film and poster; the HTML correction layer is hidden below 700px so the two label systems never duplicate or conflict. Crossing the 699px breakpoint after load must update the poster, reload the video element so its `<source media>` selection changes, and resume playback when the hero is visible. There is no caption below the film. The final `And sometimes, stuff just for fun.` is a separate italic Fraunces beat. The About portrait uses a lightweight layered fallback followed by the V2-only interactive embed inside the Renaissance arch and a 280svh desktop / 240svh mobile sticky story: paragraph two initially reads `Even when the practical value is questionable, there is value in the process.` as one normal sentence. A measured focus copy begins on the exact inline phrase, then grows through actual font-size interpolation as surrounding copy fades. Desktop keeps the phrase capped within the copy column. Mobile caps it at 2.35rem and interpolates from the real wrapped line position to a viewport-safe target; the bottom-right portrait grows to 50vw with a 210px cap, while the conclusion resolves above it at `47svh`. The shorter gold path stops at the frame rather than crossing the face. Opening the expanded construction note hides the phrase, portrait, and path, then presents the explanation in an opaque inset panel with its own capped scrolling. The section unlocks only after the process statement appears. One stable parent stage owns pointer input and passes normalized coordinates into the presentation-only iframe, preventing competing cursor and pose systems.
- Approved PNG artwork remains the editable/source archive. Public homepage, proposal preview, dashboard, selected-work, and workflow-detail surfaces use checked-in WebP derivatives at the same intrinsic dimensions. The complete responsive still set referenced by the homepage is budgeted below 4.5 MB, with a 350 KB ceiling per public still unless an exception is documented. This preserves the artwork while avoiding the delayed multi-image pop-in caused by serving the 1–2 MB PNG masters.
...
The approved Search-Optimized Content plate defines the workflow visual language: black field, off-white cards, real tool marks, realistic browser/table/document fragments, and fine blue, green, red, and gold routing. All process workflow artwork is image-generated from that reference; HTML, CSS, SVG, Mermaid, or shared flowchart recreations are not accepted source truth. Its immutable source remains `content-production-approved-desktop.png`, and the production WebP is an unchanged optimized derivative. The folder UI supplies the shared warm `#F7F6F2` artwork surface without cropping, recolouring, or replacing any workflow image. Each topology follows the actual workflow:
