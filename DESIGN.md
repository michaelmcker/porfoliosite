...
- Folder labels: local Fraunces at approximately 520 weight. Desktop uses five separate, smoothly rounded folder-tab silhouettes in permanent top-to-bottom slots on the right with a narrow gap. Labels read vertically from top to bottom and remain fully readable without hover. Selection never moves their resting slots; hover and focus lift one tab 12px outward. The tab layer remains above the paper, while the contained active paper card sits to the left as an independent object. Below 1100px preserve the existing horizontal labels on overlapping vertical folder rows.
...
- The neutral workflow accordion becomes vertical below 1100px. On desktop, all five discrete shaped tabs are absolutely anchored in a right-hand column. The current paper retracts rightward behind them for 200ms, then the selected paper begins on the right and slides left into place over roughly 520ms with `cubic-bezier(.22, 1, .36, 1)`. Inactive papers remain hidden and cannot intercept input. Mobile and reduced-motion selection commit immediately and keep the active row visible as the accessible control.
...
The approved Search-Optimized Content plate defines the workflow visual language: black field, off-white cards, real tool marks, realistic browser/table/document fragments, and fine blue, green, red, and gold routing. All process workflow artwork is image-generated from that reference; HTML, CSS, SVG, Mermaid, or shared flowchart recreations are not accepted source truth. Its immutable black-canvas source remains `content-production-approved-desktop.png`; the desktop warm-paper UI uses `content-production-folder-card.png`, cropped from the approved generated folder reference so its black interface cards are opaque rather than transparent knockouts. Each topology follows the actual workflow:
