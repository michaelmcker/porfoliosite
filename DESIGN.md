...
- The full-bleed hard-gold contact chapter gathers actual portfolio objects into a deterministic, reversible scroll spiral. Crossing the release threshold transfers those exact positions to locally vendored Matter.js bodies, which fall, collide, settle, and become draggable/tossable. Once settled, `Systematise.` morphs into `Let’s build something useful.` and the contact actions appear; continued scroll then releases the sticky stage into the footer. Reduced-motion and script failure preserve a complete static composition.
