...
- Folder labels: local DM Sans at approximately 820 weight. Desktop uses narrow 88px charcoal spines in permanent right-hand slots with the labels reading vertically; selection never moves a spine or its label. Hover and keyboard focus may lift a spine toward the reader. The active paper card sits to their left as an independent object. Below 1100px preserve the existing horizontal labels on overlapping vertical folder rows.
...
- The neutral workflow accordion becomes vertical below 1100px. On desktop, all five spines are absolutely anchored on the right. The current paper retracts rightward behind them for 200ms, then the selected paper begins on the right and slides left into place over roughly 520ms with `cubic-bezier(.22, 1, .36, 1)`. Inactive papers remain hidden and cannot intercept input. Mobile and reduced-motion selection commit immediately and keep the active row visible as the accessible control.
