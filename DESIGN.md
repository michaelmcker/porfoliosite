...
The workflow accordion is deliberately neutral and dark: Repeatable Systems is a full-bleed charcoal chapter with five shaped charcoal folder tabs fixed in one horizontal row across the top and warm-white vertical labels. The active content is one loose paper-grey `#E7E8E3` card below that travels independently, never a sheet inside a folder holder. Workflow detail pages use the same paper-grey artwork surface. Full-background exports retain their complete authored canvas; background-removed or transparent assets must reveal paper grey, never black. Bias to Build uses hard gold and About uses deep forest. The Hero and Selected Work are white; a contained hairline aligned to the navbar width separates those chapters without creating a full-bleed colour block. Project stages remain transparent. Music is the other black interlude.
...
- Folder labels: local DM Sans at approximately 820 weight. Desktop uses five wide folder-tab silhouettes with raised lobes and curved shoulder cutouts in permanent left-to-right slots across the top. Labels read vertically; selection, hover, and focus never move a tab or label. The active paper card sits below as an independent object. Below 1100px preserve the existing horizontal labels on overlapping vertical folder rows.
...
- The neutral workflow accordion becomes vertical below 1100px. On desktop, all five shaped tabs are absolutely anchored across the top row. The current paper retracts rightward behind them for 200ms, then the selected paper begins on the right and slides left into place over roughly 520ms with `cubic-bezier(.22, 1, .36, 1)`. Inactive papers remain hidden and cannot intercept input. Mobile and reduced-motion selection commit immediately and keep the active row visible as the accessible control.
