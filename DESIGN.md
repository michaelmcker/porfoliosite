...
- The full-bleed hard-gold contact chapter gathers actual portfolio objects into a deterministic, reversible full-frame orbit. The route enters from the upper left, curves down the left and across the bottom, rises at the right, and arches into the upper middle. At `0.68` progress, those exact positions become locally vendored Matter.js bodies for one short gravity drop with restrained bounce and rotation. The contact message resolves about 420ms into the drop and dynamically changes between dark and light treatments when objects cross it. The finale portrait layers its registered head pose over the body plate. After the initial settle, manually placed objects pin to their release coordinates instead of falling, bouncing, or spinning again. Continued scroll releases the sticky stage into the footer; reduced motion and script failure preserve a complete static composition.
