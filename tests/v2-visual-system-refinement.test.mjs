...
test("workflow routes use responsive artwork from the approved Content visual family", async () => {
  ...
  assert.match(homepage, /assets\/workflows\/content-production-folder-card\.webp/);
  await access(new URL("v2/assets/workflows/content-production-approved-mobile.webp", repo));
  await access(new URL("v2/assets/workflows/content-production-folder-card.webp", repo));
  ...
  assert.match(contentDetail, /\.\.\/assets\/workflows\/content-production-folder-card\.webp/);
...
test("approved Content style drives image generation while Content and the recovered real Dashboard stay locked", async () => {
  ...
  const lockedAssets = new Map([
    ...
    ["v2/assets/workflows/content-production-approved-desktop.webp", "d8b184108ea27f184755633274999909c6e62ec048683ab019cc7820587c7144"],
    ["v2/assets/workflows/content-production-folder-card.png", "5b06be1b1728b663409ebbbd5b12845bb4e6b0ba5839dca81d5a1327a94b1bca"],
    ["v2/assets/workflows/content-production-folder-card.webp", "8615154130715bd4841d23657e57ac5e668c6b7a2c09b11b2c5ecd9a65f3ef35"],
  ]);
...
test("desktop workflow controls restore the live sliding rails without covered labels", async () => {
  ...
  assert.match(css, /\.workflow-item\.is-active\s*\{[^}]*flex-basis:\s*calc\(100% - 256px\)[^}]*overflow:\s*hidden/s);
  assert.match(css, /\.workflow-trigger\s*\{[^}]*width:\s*76px[^}]*min-width:\s*76px/s);
  assert.match(css, /\.workflow-trigger-label\s*\{[^}]*font-size:\s*1\.2rem[^}]*writing-mode:\s*vertical-rl[^}]*transform:\s*rotate\(180deg\)/s);
  assert.match(css, /\.workflow-item\.is-active \.workflow-trigger\s*\{[^}]*width:\s*0[^}]*opacity:\s*0/s);
  assert.match(css, /\.workflow-trigger:hover,[^}]*\{[^}]*translate3d\(-18px,\s*var\(--stack-y\),\s*40px\)/s);
  ...
  assert.match(css, /@media \(max-width: 1099px\)[\s\S]*?\.workflow-trigger-label\s*\{[^}]*writing-mode:\s*horizontal-tb[^}]*transform:\s*none/s);
});
